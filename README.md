...
**🔧 交互体验优化**
- ✨ **修复配置持久化问题**：解决Store配置异步加载导致的回显失效问题，确保应用重启后配置正确显示
- ✨ **优化用户复制体验**：移除连接状态下的禁用类，允许用户选中复制SN和发送内容
- ✨ **缩短提示框时长**：所有ElMessage提示框显示时长从3秒优化为2秒，提升交互流畅度
- ✨ **优化遮罩层显示**：修复配置面板遮罩层导致文字难以看清问题，实现鼠标穿透效果
- ✨ **滚动条交互增强**：实现悬停显示滚动条模式，轨道背景设为透明，默认隐藏滑块，提升用户体验
